		...
		for D in range(0,NUM_station):
			for peroid in range(0,NUM_peroid):
				...
				for index in range(0,7):
					...
					print(Perday_ODRRecord[Random_Order_List[day+index]][O][D][peroid],end = ' ')
...
os.chdir('C:/Users/SyuShengWei/Desktop/project/Analysis/Group_Top_30min')	
...
os.chdir('C:/Users/SyuShengWei/Desktop/project/Analysis/Group_Top10_30min')
for O in range(NUM_station):
	outFile = open('Top10-'+str(O).rjust(3,'0')+'.csv','a')
	for peroid in range(0,NUM_peroid):
		for day in range(0,NUM_group):
			for index in range(0,NUM_output):
				outFile.write(str(Top_10_Station_List[day][O][peroid][index]))
				if index != NUM_output -1 : 
					outFile.write('_')
			if day != NUM_group -1 :
				outFile.write(',')
			else :
